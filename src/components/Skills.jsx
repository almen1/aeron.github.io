...
const Skills = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative h-screen px-4 md:px-9 border-t border-b flex items-center justify-center"
      style={{
    ...
    >
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8">
        <h2
          className="font-main text-3xl md:text-6xl font-medium"
          style={{ color: "var(--color-background)" }}
        >
          MY EXPERTISE ↗
        </h2>

        <div className="grid grid-cols-2 md:grid-cols-2 gap-x-4 md:gap-x-8 gap-y-1">
          {skills.map((skill, index) => (
            <span
              key={index}
              className="skill-item font-main text-xs md:text-sm uppercase tracking-wide"
              style={{ color: "var(--color-background)" }}
            >
              {skill}
            </span>
